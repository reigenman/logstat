...
struct ProgramOpts {
    ...
    void SetThreads(const char * value)
    {
        ...
        try {
            ...
            threadNumber = static_cast<unsigned>(num);
...
int main(int argc, char * argv[])
{
    ...
    try {
        logstat::app::Application app(opts.threadNumber);
        app.Run(fs::path(opts.logDirPath), fs::path(opts.outFilePath), opts.logFilePrefix);
        return 0;
    } catch (const std::exception & e) {
